 ...
 *
 * Launched via: run 1   (module index 1, after init.elf at index 0)
 *
 * Demonstrates:
...
{
    if (ev->type == GUI_EVENT_MOUSE_BTN &&
...
{
    (void)ev;
    /* No interactive elements — uptime refreshes on every loop iteration */
}
...
void _start(void)
{
    ...
    for (;;) {
        ...
        /* Refresh uptime window every ~500ms */
        unsigned int now = (unsigned int)uptime();
        if (win2 >= 0 && now - last_uptime_draw >= 500) {
